...
// actorContextKey is the unexported context key the surveys services
// use to pull the acting user id when emitting audit rows. The HTTP
// transport (Plan 07 Task 6) populates it from the JWT claims; tests
// inject the actor via WithActorID directly.
type actorContextKey struct{}

// WithActorID returns a context that carries the supplied actor user
// id. The surveys service inspects the context for this value when
// writing audit rows; an absent value -> ActorKind=system, nil ActorID
// (system bootstrap or worker-driven action).
...
// actorIDFromContext returns the actor id stored on ctx by
// WithActorID, or a nil pointer when no actor is present.
...
// at every call site. The surveys module composition root registers a
// real logger (or the noop fallback when audit hasn't registered yet);
// tests inject a fakeAudit fixture that records every Write.
func (s *SurveyService) writeAudit(ctx context.Context, ev auditapi.Event) error {
	if s.audit == nil {
		return nil
	}
	if ev.ActorKind == "" {
