...
// Test helpers shared between the crm store integration tests. The
// helpers boot Postgres 16 in a container, apply the project's full
// migration set (000001 + 000002 + 000003 + 000004 + 000005), and yield
// a *postgres.Pool wired against it. The shape mirrors
...
// newCRMTestPool boots Postgres 16 in a container, applies the project
// migrations through 000005, opens a *postgres.Pool against it, and
// registers t.Cleanup to tear everything down.
func newCRMTestPool(t *testing.T) *postgres.Pool {
	t.Helper()
...
// The crm/store integration tests need a real tenant FK target so
// projects.tenant_id satisfies the existing reference constraint.
