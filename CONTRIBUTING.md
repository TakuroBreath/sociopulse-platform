...
7. **Modernize** — Go 1.26+ idioms: `any` over `interface{}`, `min/max/clear` builtins,
   `range` over int (Go 1.22), `slices`/`maps` packages, `t.Context()` in tests
   (Go 1.24), `cmp.Or`, `wg.Go()` (Go 1.25), `sync.OnceFunc/OnceValue`
...
The `docker` job in `.github/workflows/ci.yml` pushes images to Docker Hub
(`maxtakuro/sociopulse-api`). For the push to succeed, the following GitHub
repository secrets must be configured (Settings → Secrets and variables →
Actions):

| Secret | Value | Where to get it |
|---|---|---|
| `DOCKERHUB_USERNAME` | `maxtakuro` | The Docker Hub account name. |
| `DOCKERHUB_TOKEN` | A Personal Access Token | https://hub.docker.com/settings/security → "New Access Token" → scope **Read & Write**. Copy the value once (it's hidden after). |

If the secrets are missing, the `docker` job still **builds** the image (so PRs
and `main` builds aren't blocked) — it only skips the **push** step and emits
a workflow notice. You can add the secrets later and the next push will start
publishing.

Other secrets used by the workflow:

- `GITHUB_TOKEN` — auto-generated by GitHub for `gitleaks`, `osv-scanner`,
  CodeQL. No manual setup needed.
