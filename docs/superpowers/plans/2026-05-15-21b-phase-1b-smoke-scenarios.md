...
- **Task 1 (190a0d9)** — `BuildCSVImport_FormatMatches` self-test substitutes `encoding/csv` round-trip + header check for the unexported `internal/crm/service/import.parseCSV` (originally cited in plan; the function has no public surface, so direct call would require an export-for-test seam). The self-test still pins the CSV format contract: header literal `phone` present + row count matches input. End-to-end correctness is verified by Task 3.
- **Task 1 (190a0d9)** — `SeedRecording` accepts `objects storage.ObjectStore` as an explicit parameter rather than reaching into `cmd/api.GetSmokeRecordingPorts()` directly (forced by Go's package model — `tests/smoke` can't import `package main`). Caller wires the override seam by hand: `objs := GetSmokeRecordingPorts().Objects; smoke.SeedRecording(t, stack, objs, ...)`.
- **Task 1 (190a0d9)** — TRUNCATE list in `Stack.Reset` adds `call_events` + `project_dnc` (not in plan's reference-doc skeleton, but FK-cascade-related to listed tables) and OMITS `respondent_imports` (table doesn't exist — asynq import job state is Redis-only) + `audit_log` (preserved for cross-test audit assertions; Plan 21b Task 6 uses `NoopAuditLogger` so the issue is moot for that scenario).
- **Task 1 fix-up (3d94662)** — 4 HIGH fixes from code-quality review applied in single fix-up commit: (1) `tests/smoke/main_test.go` added with `goleak.VerifyTestMain` + `TerminateOnTestMainCleanup` (smoke binary previously orphaned pgPool + testcontainers); (2) `smokeOverrideRecordingPorts` switched from plain `*recwire.Ports` to `atomic.Pointer[recwire.Ports]` (`-race` foot-gun fix); (3) `TestHarness_FutureClock_Returns_AddedDuration` rewritten to two-sided bounds (previous assertion was tautological — `now+offset > now+offset-1s` always true); (4) `TestHarness_Stack_Reset_TruncatesSeededRow` dropped `t.Parallel` (TRUNCATE is container-global; concurrent inserts would race). Plus 5 MEDIUM (test renames, testifylint nits, `strings.Join` over hand-rolled, `Stack.PgPool` ctx fix, `Stack.Reset` uses cached pool) + 6 LOW (errors.New, sync.Once on Close, named constants, comment cleanups). `Stack.Reset` uses `pool.RawExec` (not `BypassRLS`) because `tenancy_admin` deliberately lacks TRUNCATE grants on `call_recordings` — documented trade-off.
- **Task 2 (a9b2d16)** — Plan said "0 production code changes" but new migration `000015_admin_grants_surveys.{up,down}.sql` was provably required: surveys-side `RequireSameTenant` middleware uses `pool.BypassRLS` and `tenancy_admin` had no SELECT grant on `surveys` / `survey_versions` (42501 → middleware 500). Mirrors canonical 000009 / 000011 / 000014 pattern; tight grant set (SELECT-only). Wire-format corrections vs plan text: `CreateSurveyRequest` has no `code` field (uses `name` + optional `description` / `primary_mode`); `SaveVersionResponse` is full `VersionDTO` with separate `major` (int) + `minor` (int) (not `{version_id, semver}`); activate returns 204 No Content (not 200).
- **Task 3 (1d2ec3a)** — Plan said "0 production code changes EXPECTED" but four files touched (anticipated by references file § 4.5 which designated Task 3 as the KEK pre-registration site): `pkg/config/kms.go` adds `LocalKEKs map[string]string`; `internal/tenancy/store/kms_client_local.go` adds `NewLocalKMSClientWithKEKs(masterHex, preKEKs)` constructor; existing `NewLocalKMSClient(masterHex)` becomes 1-line wrapper (backward compatible — no caller breaks); `internal/tenancy/module.go::buildKMSClient` routes through it; `tests/smoke/config.go` registers `kms.local_keks: { smoke-kek-default: <64-hex same as recording.local_keks> }`. No new tenancy_admin grant migration needed — import path uses `pool.WithTenant` (not BypassRLS), and migration `000001_init.up.sql` already grants app_user CRUD on respondents + project_dnc + projects.
- **Task 4 (cf0f2e6)** — test-only commit (no production touch). Wire-format reality flagged for future scenarios: dialer's `/api/operator/ws` does NOT publish initial snapshot on Accept (`pumpSnapshots` only forwards AFTER Subscribe registers); the wire frame is bare `SnapshotDTO` JSON (no `{type:...}` envelope); state literal is `"pause"` (not `"paused"` per FSM constants); POST /api/sessions/start REQUIRES upfront project assignment (`operator_sessions.project_id` NOT NULL FK); POST /api/sessions/pause REQUIRES `{"reason":"..."}` body. Per-test t.Cleanup ordering hardened to DELETE `operator_sessions` BEFORE `tenants` (FK is NOT ON DELETE CASCADE).
- **Task 5 (a2b78f6)** — production-code change: `internal/recording/transport/http/recording_handler.go` now actively rejects `Range:` requests (was only advertising `Accept-Ranges: none`). Per ADR-0005 §15.4 + RFC 7233 §4.4: any non-empty Range → 416 + `Content-Range: bytes */*` + `recording.range_not_satisfiable` envelope. Short-circuits BEFORE the storage lookup. New unit test `TestStreamRecording_RangeRejected` pins the contract at handler layer; smoke is the end-to-end regression net. Wire-format reality: Search response field name is `"sha256"` (not `"sha256_hex"` despite the DB column being `sha256_hex`); stream `Content-Type: audio/ogg` (per `service.contentTypeForCodec` mapping `opus → audio/ogg`); cross-tenant 404 envelope code is `recording.not_found`.
- **Task 6 (64673bc)** — `cmd/api/main_test.go` build-tag-split into `testmain_default_test.go` (`!smoke`, original `goleak.VerifyTestMain`) + `testmain_smoke_test.go` (`smoke`, manual `m.Run` → `smoke.TerminateOnTestMainCleanup` → `goleak.Find`). The split is the canonical fix Task 4 deferred — drains pgxpool + testcontainers BEFORE leak inspection. PurgeWorker construction uses existing clock seam (`internal/crm/service/purge.go:66-101`); no production-code refactor needed. Helper `smokeHTTPDeleteWithJWT` added in `smoke_purge_test.go` (Plan-21 helpers were `postWithJWT` / `getWithJWT`; if a third scenario needs DELETE, helpers can consolidate in a refactor).
- **Phase-1c follow-up captured (NOT in scope)** — `-count=N>1` smoke runs race on JetStream stream re-creation (parallel scenarios + `EnsureSmokeStreams` `t.Cleanup(DeleteStream)`). Canonical CI gate is `-count=1`; 10 scenarios deterministically green there. Fix: lift `EnsureSmokeStreams` out of `bootAPI` into `newSharedStack` (one provision per `TestMain`) OR cleanup-as-no-op (testcontainers reaper drops streams at process exit).
- **Pre-existing flake observed (NOT a Plan-21b regression)** — `pkg/passwords/argon2id_test.go::TestVerify_TimingDeltaSmall` is timing-sensitive; passes on retry. `pkg/passwords/` not touched by Plan 21b. Future hardening: widen 10% threshold to 25% OR `t.Skip` on shared CI.
