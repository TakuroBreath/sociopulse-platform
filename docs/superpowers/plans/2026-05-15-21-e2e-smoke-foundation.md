...
- **Task 1 (`0c3e747`)** — added an extra unit assertion. The plan permitted "providers list contains tenancy", but the implementer pinned BOTH presence AND first-position (`providers.Modules[0].Name() == "tenancy"`) because reordering tenancy down the slice breaks every downstream consumer silently. The test was renamed mid-review to `TestBuildProviders_TenancyIsFirstEntry` (the plan-hinted `TestRun_TenancyLocator*` name implied a runtime locator-entries check, which doesn't happen in the unit-test path).
- **Task 1 (`5a05c19`, code-quality review fixup)** — added explicit `require.NotNil(t, tenancyapi.Register, ...)` assertion before checking the providers slice. The original docstring claimed the test caught a missing blank-import; in reality the slice-only check did not. The new assertion makes the docstring claim true.
- **Task 2 (`b81e527`)** — scope expanded: implementer had to fix an asynq shutdown-race nil-pubsub panic in `internal/crm/module.go` (switched constructors to `*FromRedisClient` shared-connection variants). The panic was deterministic-on-shutdown the moment crm.Module joined the providers walk. Without the fix, the boot test could never go green. Refactor is small (~10 LoC) but lives outside the spec's `cmd/api/` file list.
- **Task 2** — implementer flagged that `JWTConfig.Secret` has `mapstructure:"-"` (so the YAML key added in this task is documentary-only at runtime). Resolved in the Task 6 bridge fix below.
- **Task 3 (`2a403e0`)** — added migration `000014_admin_grants_calls.{up,down}.sql` (out of the original file list). Required because `pool.BypassRLS` switches role to `tenancy_admin`, which had no grants on `calls` (mirrors Plan 12.4's migration `000011` for `call_recordings`). Without this migration the new `PgCallTenantResolver` fails with `42501 permission_denied`. Also added 3 testcontainers integration tests pinning the BypassRLS cross-tenant invariant + the grant.
- **Task 4 (`fbe9fad`, fixup)** — `tests/smoke/stack.go::init()` disables `TESTCONTAINERS_RYUK_DISABLED` before any container starts. The implementer's initial Task 4 commit (`76a77a8`) failed on macOS Docker with a `goleak` false-positive on `Reaper.connect.func1`. Plan references § 4.1 had flagged this; the README mentioned the workaround but the code didn't apply it.
- **Bridge fix (`0e3e63c`, between Tasks 5 and 6)** — TWO coupled changes that surfaced together once Plan 21 Task 2 unblocked auth wiring: (a) `pkg/config/auth.go::JWTConfig.Secret` lost the `mapstructure:"-"` tag so viper actually loads it from YAML/env (`SOCIOPULSE_AUTH_JWT_SECRET`); production hygiene now enforced by `cfg.Validate()` only. (b) Renamed `/api/calls/:call_id/recording{,/verify}` → `:id/recording{,/verify}` in `internal/recording/transport/http/` to resolve gin's trie panic (dialer's `/api/calls/:id/{status, hangup}` was always there; conflict was latent until auth.Module's activation triggered the full providers walk). Both fixes are minimal; zap field names + JSON tags (`json:"call_id"`) unchanged — those are wire/log contracts, independent of URL routing.
- **Task 6 (`a07b385`)** — KMS handling path: chose "do nothing" (neither config Option A nor seed Option B from the spec). The auth login flow does NOT touch KMS, so a dangling `kms_kek_id` in the seeded tenant is benign. Future scenarios that DO decrypt under tenant KEK (recording stream, phone-hash lookup — both deferred to Phase-1b) will need to mint or pre-register a KEK at seed time.
- **Task 6** — replaced `readBody(t, resp)` (used inside `Equalf` variadic args, which evaluates eagerly and drains the body) with a `postJSONAndRead(...)` helper that consumes the body once into bytes. Plan spec snippet had the eager-eval bug; implementer caught it before commit.
- **Task 7 (`9476831`)** — `SeedProject` inserts `customer=''` (empty string) rather than NULL because `internal/crm/store/project_store.go::scanRow` reads the column into a plain `string` — `pgx.Scan` fails on NULL and the BypassRLS resolver returns a generic 500 instead of the expected 404. This is the minimum that satisfies both the storage path and the `RequireSameTenant` guard. Documented inline in the seed function godoc.
- **Task 7** — chose `POST /api/projects` as the RBAC test endpoint (over the spec's fallback option of `POST /api/auth/users`). Body shape `{code, name}` is well-defined and admin-only via both transport `requireAdminRole()` middleware and the static RBAC matrix.
- **Final review skipped** — per re-review proportionality heuristic (`docs/architecture/09-agent-workflow-improvements.md` § Improvement #7), the 8 tasks accumulated 1 MEDIUM (dead `errs` slice in `crm.Module.Stop`) + multiple LOWs across the spec/code-quality reviews; all addressed inline. The "final code reviewer over `origin/main...HEAD`" stage was not separately dispatched because every task ended with `make ci` + `go test -tags=smoke ...` reality-check on the controller side and the diff scope of each task review was sufficient.
